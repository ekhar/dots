...
# Cross-platform calendar script
# On macOS: Uses icalBuddy for calendar integration
# On Linux: Shows a simple icon (no calendar integration)

# Check if we're on macOS and have icalBuddy
if [[ "$(uname)" != "Darwin" ]] || ! command -v icalBuddy &>/dev/null; then
    echo "󱁕 "
    exit 0
fi

# macOS with icalBuddy - full calendar integration
